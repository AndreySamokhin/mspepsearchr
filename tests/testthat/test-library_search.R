...
test_that("Library search, synthethic mass spectra", {
...
test_that("Library search, alkanes_ei_lr.msp", {
...
test_that("Library search, alkyl_fragments_msms_hr.msp", {
...
test_that("Library search, chlorine_compounds_ei_hr.msp", {
...
test_that("Library search, chlorine_compounds_ei_lr.msp", {
...
test_that("Library search, mw288_compounds_msms_hr.msp", {
...
test_that("Library search, 'presearch' argument (mw)", {
...
test_that("Library search, 'presearch' argument (inchikey)", {
...
test_that("Library search, 'n_hits' argument", {
  hl <- SimilaritySearchEiSimple(list(dummy_spectrum), lib_path,
                                 n_hits = 3L, temp_dir = temp_dir)
  expect_true(nrow(hl[[1L]]) == 3L)
  hl <- SimilaritySearchEiSimple(list(dummy_spectrum), lib_path,
                                 n_hits = 5L, temp_dir = temp_dir)
  expect_true(nrow(hl[[1L]]) == 5L)
})



#==[ Inline tests ('ri_column_type') ]==========================================
...
test_that("Library search, 'ri_column_type' argument", {
  ...
  expect_identical(hl$ri[new_order],
                   c(1100L, NA_integer_, 1433L, NA_integer_, 1400L, 1300L,
  ...
  new_order <- order(hl$id)
  expect_identical(hl$ri[new_order],
                   c(1100L, NA_integer_, 1434L, 1938L, 1400L, 1300L, 1200L,
                     2334L, 1100L, 1200L))
  expect_identical(hl$ri_type[new_order],
                   c("a", "", "s", "s", "a", "a", "a", "s", "a", "a"))
  ...
  new_order <- order(hl$id)
  expect_identical(hl$ri[new_order],
                   c(1100L, NA_integer_, NA_integer_, 2688L, 1400L, 1300L,
  ...
  new_order <- order(hl$id)
  expect_identical(hl$ri[new_order],
                   c(1100L, NA_integer_, 1433L, 1938L, 1400L, 1300L, 1200L,
                     2296L, 1100L, 1200L))
  expect_identical(hl$ri_type[new_order],
                   c("a", "", "n", "s", "a", "a", "a", "n", "a", "a"))
...
#==[ Inline tests ('load_in_memory', 'n_threads') ]=============================

msp_file <- system.file("extdata/spectra/alkanes_ei_lr.msp",
                        package = "mspepsearchr")
lib_path <- system.file("extdata/libraries/massbank_subset_ei_lr/",
                        package = "mspepsearchr")
res_identity <-
  IdentitySearchEiNormal(msp_file, lib_path, load_in_memory = TRUE)
res_similarity <-
  IdentitySearchEiNormal(msp_file, lib_path, load_in_memory = TRUE)
...
test_that("Library search, 'load_in_memory' argument", {
  hl <- IdentitySearchEiNormal(msp_file, lib_path, load_in_memory = FALSE)
  expect_identical(hl, res_identity, ignore_attr = TRUE)
  hl <- IdentitySearchEiNormal(msp_file, lib_path, load_in_memory = FALSE)
  expect_identical(hl, res_similarity, ignore_attr = TRUE)
})


#.. 'n_threads' ...............................................................#

test_that("Library search, 'n_threads' argument", {
  skip_on_ci()
  n_threads <- parallel::detectCores()
  skip_if(n_threads < 2L, "Skipping: fewer than 2 threads available.")

  hl <- IdentitySearchEiNormal(msp_file, lib_path, n_threads = 2L)
  expect_identical(hl, res_identity, ignore_attr = TRUE)
  hl <- IdentitySearchEiNormal(msp_file, lib_path, n_threads = 2L)
  expect_identical(hl, res_similarity, ignore_attr = TRUE)
})
...
#==[ Inline tests (duplicated CLI flags) ]======================================

test_that("Library search, duplicated CLI flags", {
msp_file <- system.file("extdata/spectra/alkanes_ei_lr.msp",
                        package = "mspepsearchr")
lib_path <- system.file("extdata/libraries/massbank_subset_ei_lr/",
                        package = "mspepsearchr")
expect_error(
  IdentitySearchEiNormal(msp_file, lib_path, addl_cli_args = "/HITS 10"),
  "The following CLI flags are duplicated: /HITS"
)
expect_error(
  IdentitySearchEiNormal(msp_file, lib_path,
                         addl_cli_args = c("/OutMW", "/OutCAS")),
  "The following CLI flags are duplicated: /OutMW, /OutCAS"
)
})
